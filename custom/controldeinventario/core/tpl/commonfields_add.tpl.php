...
if($entrada){
	...
	{	
		if($x==0){
			...
			$allCost_pesos = "SELECT t_cost,iva FROM hor_controldeinventario_entradas WHERE tiporeq = ".$isstock;
			if(($yearSelect!="x"||$yearSelect>0) && $isstock>0){
				$allCost_pesos.= ' AND ref LIKE "'.$valor.'%"';
			}
			$sumaArticulosMNX=0;
