		...
		else //Si la requisicion es NO STOCK(NS), SERVICIO, ETC.
		{
			...
			if($action=='add')
			{
				...
				if (! $error) {
					...
					//actualizamos el inventario no stock, por
...
if ($action == 'create') {
	...
	//calculamos años ANTES de sumar las cosas.

	$Años_Consulta="SELECT DISTINCT YEAR(date_creation) FROM ".MAIN_DB_PREFIX."controldeinventario_entradas ;";
	...
	//aqui vemos que leemos dependiendo si es stock, o no.
	$yearSelect = GETPOST("yearSelect","alpha");
	if($yearSelect=="x"||$isstock==0)
	$tableQuery='SELECT * FROM '.MAIN_DB_PREFIX.'controldeinventario_entradas WHERE tiporeq = '.$isstock.' ORDER by ref ASC';
	else{
		if($yearSelect==0){
			$valor = $Años_Lista[count($Años_Lista)-1];
		}else{
			$valor = $yearSelect;
		}
		$yearSelect = $valor;
		switch($isstock){
			case 1:
				$valor = "NS".substr($valor,strlen($valor)-2);
				break;
			case 4:
				$valor = "OS".substr($valor,strlen($valor)-2);
				break;
			case 6:
				$valor = "ANS".substr($valor,strlen($valor)-2);
				break;
		}
		$tableQuery='SELECT * FROM '.MAIN_DB_PREFIX.'controldeinventario_entradas WHERE tiporeq = '.$isstock.' AND ref LIKE "'.$valor.'%" ORDER by ref ASC';
	}
	//y despues de eso calculamos las entradas a imprimir.
	// Common attributes
...
//Botones para el año..pero SOLO si son entradas no stock, Ordenes de servicio o agroquímicos.
if($isstock>0){
print '<div >';
	
	if(count($Años_Lista)>0){
		foreach($Años_Lista as $valor){
		
		print '<a class="yearbutton" href="?action=create&isstock='.$isstock.'&backtopageforcancel='.$backtopageforcancel.'&yearSelect='.$valor.'">'.$valor.'</a>';
		}
	}
	print '<a class="yearbutton" href="?action=create&isstock='.$isstock.'&backtopageforcancel='.$backtopageforcancel.'&yearSelect=x">Todos</a>';
