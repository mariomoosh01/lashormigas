...
if($dataToLoad!=NULL){
    ...
    if($filterToSee)
        switch($filterToSee)
        {
            ...
            case 100:
                if($endinDate=='')
                $consultasql.= " WHERE date_creation >= '".$startDate."' AND date_creation <= '".date("Y-m-d")." 23:59:59'";
                else if ($startDate=='')
            ...
            case 110:
                ...
                $consultasql.= " WHERE tms >= '1990-01-01' AND tms <= '".date("Y-m-d")." 23:59:59' AND qty > 0";
                else if($endinDate=='')
                $consultasql.= " WHERE tms >= '".$startDate."' AND tms <= '".date("Y-m-d")." 23:59:59' AND qty > 0";
                else if ($startDate=='')
                $consultasql.= " WHERE tms >= '".$endinDate."' AND tms <= '".$endinDate." 23:59:59' AND qty > 0";
                else if($startDate && $endinDate)
                $consultasql.= " WHERE tms >= '".$startDate." ' AND tms <= '".$endinDate." 23:59:59' AND qty > 0";
...
else if($dataToLoad == 'emisionderequisiciones')
{
    ...
    } else if ($filterToSee>=100 && $filterToSee<110){
        $title.="requisiciones por fecha";
    }
    else{
        $title = "requisiciones";
    }
...
if ($resql) {
    ...
    else if($dataToLoad == 'emisionderequisiciones')
    {
        $listatablas = new emisionderequisiciones($db);
         if($filterToSee==9||$filterToSee==7){
            ...
            $listatablas->fields['udm']['comment']="U.D.M";
            $listatablas->fields['uqty']['comment']="Cantidad";
            ...
            $listatablas->fields['tms']['comment']="Fecha de requisición";
            $listatablas->fields['udm']['comment']="U.D.M";
            $listatablas->fields['qty']['comment']="Cantidad";
            $listatablas->fields['date_creation']['comment']="Fecha";
        }
    ...
    else if ($dataToLoad == 'emisionderequisiciones' && ($filterToSee>=100 && $filterToSee<110))
    {
        // requisiciones por fecha
        if($orientacion=="L")
        $widts = [27,152,10,15,15,20,21];//landscape tiene 270 caracteres de largo
        else 
        $widts = [21,96,21,21,21];//portrait tiene 180 caracteres de largo
    }
    else if($dataToLoad == 'inventarions'||$dataToLoad == 'inventarioans'){
    ...
    for ($j=0;$j<count($informacion);$j++){
        for($h=0;$h<count($reduTable);$h++){
            ...
            if ($reduTable[$h]=='status'){

                $temp = $informacion[$j][$h];
                
                switch($temp){
                    case 0:
                        $informacion[$j][$h]= "Error";
                    break;
                    case 1:
                        $informacion[$j][$h]= "No surtidas";
                    break;
                    case 2:
                        $informacion[$j][$h]= "Parciales";
                    break;
                    case 9:
                        $informacion[$j][$h]= "Surtidas";
                    break;
            ...
            if($reduTable[$h]=='t_cost'||$reduTable[$h]=='cost'){
                if(in_array('iva',$reduTable))
                {
                    ...
                    $informacion[$j][$h]= round($informacion[$j][$h],2);
                }
            }
